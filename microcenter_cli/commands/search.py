...
from ..urls import VALID_RESULTS_PER_PAGE
...
def search(
    ...
    if all_pages:
        results = list(
            ctx.client().search_all(query, store, category_n=category_n, rpp=rpp_int)
        )
        meta_note = f"{len(results)} results across all pages"
    else:
        result_page = ctx.client().search_page(
            query, store, page=page, category_n=category_n, rpp=rpp_int
        )
